...
import {
  DRAG_GROUP_SELECT_AUDIO,
  DROP_GROUP_SELECT_AUDIO,
  CANCEL_GROUP_SELECT_AUDIO
} from "../../../common/config.js";

let selectionBox: HTMLElement;
let isSelecting = false;
let selStartX = 0, selStartY = 0;
let selectedImgs: HTMLImageElement[] = [];

let isDraggingGroup = false;
let dragStartX = 0, dragStartY = 0;
let hasMoved = false;  // ✅ 이동 여부 확인용
let originalPositions: { img: HTMLImageElement; x: number; y: number }[] = [];
...
export function enablePlaygroundGroupMove() {
  ...
  function onSelectMouseMove(e: MouseEvent) {
    ...
    selectedImgs = Array.from(
      playgroundEl.querySelectorAll('img[data-serial]')
...
export function getSelectedImages(): HTMLImageElement[] {
  return selectedImgs;
}
